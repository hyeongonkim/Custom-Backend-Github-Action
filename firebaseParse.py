...
cred = credentials.Certificate(os.path.expanduser("~") + "/private/serviceAccountKey.json")
...
def parseUnipass(traceNum, traceYear, nowStatus):
    ...
    parsedData.clear()
    newStatus = nowStatus
    parseURL = 'https://unipass.customs.go.kr:38010/ext/rest/cargCsclPrgsInfoQry/retrieveCargCsclPrgsInfo?crkyCn=' + str(unipass_token) + '&hblNo=' + str(traceNum) + '&blYy=' + str(traceYear)
    req = ''
    while req == '':
        try:
